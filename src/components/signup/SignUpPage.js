...
const SignupPage = () => {
  ...
  const RegisterNewUser = async () => {
    ...
    const AccountCreated = await registerUser(
      firstname,
      lastname,
      phone,
      email,
  ...
  const SignupHandler = async () => {
    ...
    if (email.length < 8) {
      setSnackbarState(
        "Email should be greater than 8 characters !",
        "warning",
        true
      );
      return;
    ...
    if (emailAlreadyRegistered === true) {
      setSnackbarState(
        "Provided Email Address is Already Registered !",
        "warning",
        true
      );
      return;
  ...
  const handleSignUpWithGoogle = async () => {
    try {
      ...
      const existsAlready = await userExists();
      if (existsAlready) {
        setSnackbarState(
          "Provided Email is Already Registered ! Login Now",
          "warning",
          true
        );
        return;
      } else {
        // ✅ Set session cookies manually here
        Cookies.set("email", email, { expires: 7 });
        Cookies.set("googleSignedIn", true, { expires: 7 });
    ...
    <>
      ...
      <div className="bg-white min-h-screen pt-10 flex flex-col items-center">
        ...
        <div className="mt-5">
          ...
          <Link
            to="/login"
            className="text-[#4d41e3] text-base font-bold underline cursor-pointer ml-2">
            Login Now
          </Link>
        </div>
      </div>
